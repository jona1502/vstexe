...
microphone list. VocalChain ships such a driver, but it is not release-signed,
...
Until then VocalChain uses a virtual audio cable: an already signed third-party
driver that exposes a render endpoint and a capture endpoint wired together.
VocalChain writes the processed chain into the render side, and Discord, OBS or
any other application reads the capture side as a microphone.

```text
physical microphone -> VocalChain (VST3 chain) -> cable render endpoint
                                                       |
...
are recognised as well. VocalChain does not bundle any of them; each is
installed and licensed separately by its vendor.

## Routing

1. Start VocalChain and select the physical microphone as the input device.
...
you cannot also hear it through headphones from within VocalChain. Audio
...
is inserted between VocalChain and the reading application.
