...
int main()
{
    ...
    expect(inputrack::filterAndSortPlugins(all, "", Sort::sortAlphabetically).size() == 5,
    ...
    const auto byVendor = inputrack::filterAndSortPlugins(all, "fabfilter", Sort::sortAlphabetically);
    ...
    const auto byName = inputrack::filterAndSortPlugins(all, "PRO-Q", Sort::sortAlphabetically);
    expect(byName.size() == 1 && byName[0].name == "Pro-Q 3", "matching ignores case");

    expect(inputrack::filterAndSortPlugins(all, "nothing here", Sort::sortAlphabetically).isEmpty(),
           "a search without matches yields an empty list");

    // Alphabetical order ignores the manufacturer entirely.
    expect(namesOf(inputrack::filterAndSortPlugins(all, "", Sort::sortAlphabetically))
    ...
    expect(namesOf(inputrack::filterAndSortPlugins(all, "", Sort::sortByManufacturer))
                   .joinIntoString(",") == "Pro-C 2,Pro-Q 3,Nectar,Anthem,Orphan Tool",
           "sorting by manufacturer groups and orders within the group");

    expect(namesOf(inputrack::filterAndSortPlugins(all, "", Sort::sortByCategory))
    ...
    expect(inputrack::pluginDisplayName(all[0], Sort::sortAlphabetically) == "Pro-Q 3",
           "alphabetical entries show the bare name");
    expect(inputrack::pluginDisplayName(all[0], Sort::sortByManufacturer) == "FabFilter - Pro-Q 3",
           "manufacturer entries carry their vendor");
    expect(inputrack::pluginDisplayName(all[0], Sort::sortByCategory) == "EQ - Pro-Q 3",
